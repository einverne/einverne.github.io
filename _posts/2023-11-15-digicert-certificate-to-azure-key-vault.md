...
[[CSR 证书签名请求文件]] （Certificate Signing Request），即证书签名请求文件，证书申请人在申请数字证书时由 CSP(加密服务提供者)在生成私钥的同时也生成证书请求文件，证书申请人只需要把 CSR 文件提交给证书颁发机构后，证书颁发机构使用其根证书私钥签名就生成了证书公钥文件，也就是颁发给用户的证书。
...
- 生成 [[CSR 证书签名请求文件]] 文件，提供给 DigiCert
