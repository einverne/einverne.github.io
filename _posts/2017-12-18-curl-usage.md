...
`curl -f` 可以用来检测 HTTP 请求返回 4xx/5xx 错误，curl 会直接报错并返回非 0 退出码，不会将错误页面内容作为正常输出，`-f` 也叫做 `--fail`，适合在脚本中判断请求是否成功。
...
如果返回 404，curl 会失败退出，这样就可以在 Shell 脚本中控制后续逻辑。如果不加 `-f` 参数，curl 在遇到 404/500 错误时，也会将服务器返回的错误页面内容输出。
